...
Monitor::Monitor(zmq::socket_t& insock, Store& invariables) {
   ...
   // endpoint and port to bind monitoring socket to
...
bool Monitor::Send() {
   ...
   monitoring_data.Set("time",
                       boost::posix_time::to_iso_extended_string(boost::posix_time::second_clock::universal_time()));
...
void Monitor::CollectData() {
   ...
   if(state == 3)
      status << "Taking_Data";
   status << ",Data:Sent:Total/Capacity=" << data_queue << ":" << sent_queue << ":" << data_queue +
      sent_queue << "/" << max_queue;
